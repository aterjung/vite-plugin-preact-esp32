"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }var __create = Object.create;
...
function espViteBuild(configParams = {
  logging: false
}) {
  let assets = [];
  function addAsset(filename, data, options) {
    const path2 = `${options.dir}/${filename}`;
    const mimeType = import_mime_types.default.lookup(path2);
    if (!mimeType) {
      console.log("No mime type found for", path2);
      return;
    }
  ...
  function optimizeAsset(sourceData) {
    let response = "";
    let contents = _zlib.gzipSync.call(void 0, sourceData);
  ...
  function createESPOutputFile(options) {
    const templatePath = _path2.default.resolve(__dirname, "static_files_h.ejs");
    import_ejs.default.renderFile(templatePath, { files: assets }, {}, (err, str) => {
      if (err) {
        console.error("Error rendering EJS File:", err);
        return;
      }
      const outputPath = _path2.default.join(
        _nullishCoalesce(options.dir, () => ( "")),
        "_esp32",
        "static_files.h"
      );
      const dirPath = _path2.default.dirname(outputPath);
      if (!_fs.existsSync.call(void 0, dirPath)) {
        _fs.mkdirSync.call(void 0, dirPath, { recursive: true });
      }
      _fs.writeFileSync.call(void 0, outputPath, str);
...
exports.espViteBuild = espViteBuild;
